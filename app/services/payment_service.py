...
class ChargePaymentState:
    ...
    #: деньгах человека.
    overpaid_minor: int
    #: Остаток не закрыт ничем: ни деньгами, ни приложенным чеком. Отдельно от
    #: `is_overdue`, потому что срок тут ни при чём — это ответ на вопрос «нам
    #: ещё должны?» в любой день месяца. Выставлено наружу ради tsk-673: свод
    #: при переводе на «Выпускника» спрашивает ровно это, и своя копия формулы
    #: «чек гасит долг, только если покрывает остаток» разъехалась бы с той,
    #: по которой красится бейдж в кабинете.
    is_unpaid: bool
