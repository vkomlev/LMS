...
async def apply_browser_timezone(
    ...
    """Записать пояс, снятый с браузера, ЕСЛИ у человека его ещё нет (tsk-588, tsk-753).

    Правило приоритета (решение оператора 2026-08-08): ручной выбор сильнее
    системного пояса устройства.

    tsk-753 сузил правило до «автозахват только заполняет пустое». Прежняя
    редакция обновляла и своё же прежнее значение — и пояс переписывался при
    входе с любого чужого или рабочего компьютера, молча. Так у преподавателя
    оказался записан Екатеринбург вместо Красноярска, а рядом со временем
    занятия ей показали «(у вас 16:00)» — она прочла это как утверждение
    платформы и решила, что сломалось время. Переезд и поездка от этого не
    теряются: расхождение кабинет показывает вопросом, и ответ человека
    приходит сюда уже как ручной выбор (`PATCH /me` → `manual`).

    - пусто → записываем, источник `auto`;
    - пояс уже есть (любой источник) → не трогаем, `applied=false`.
