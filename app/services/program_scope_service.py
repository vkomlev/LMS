...
def _trim_core(
    ...
    * **начатые** (`started`) — отнять номер, в котором человек уже что-то
    ...
    for cid in ranked:
        ...
        if not cut and kept_size + size <= budget:
            kept_size += size
            continue
        # Дальше — всё, строго по приоритету. Соблазн пропустить неместившийся
        # номер и взять следующий (вдруг он меньше) выглядит бережливо, но
        # ломает сам смысл порядка: замер 05.09 показал, что так в программе
        # остаётся «Задание 27» (9 заданий) вместо «Задания 14» (50), то есть
        # выпускник учит трудное и бросает простое. Порядок задал методист —
        # система не вправе его пересобирать ради заполнения остатка.
        cut = True
        excluded.add(cid)
