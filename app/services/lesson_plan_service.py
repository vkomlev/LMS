...
async def _load_unasked_absences(
    ...
    tsk-1057/tsk-1086: ученик мог не отказаться от одного часа, пропустить
    его, но прийти к ТОМУ ЖЕ преподавателю на другое занятие — преподаватель
    его уже видел живьём, спрашивать не о чем. Решение оператора: тот же
    преподаватель, БЕЗ требования «тот же день» (22.09, живой кейс — Денис
    Ильин, тот же день; 23.09, живой кейс — Андрей Залетов, отработка на
    следующий день — исходное ограничение tsk-1057 «тот же день» его не
    закрывало). Направление намеренно одностороннее — считается только
    занятие ПОСЛЕ пропущенного (``lo2.scheduled_at >= lo.scheduled_at``):
    если разрешить «до», у любого регулярно ходящего ученика нашлось бы
    прошлое посещение и свежий пропуск гасился бы даже раньше, чем ученик
    реально пришёл отрабатывать (подтверждено на Залетове — у него уже были
    посещения ДО пропуска 22.09, и они не должны его прощать задним числом).
    Верхняя граница — строго ``< before`` (не ``<=``): САМ просматриваемый
    occurrence (`before` — его же ``scheduled_at``) не считается доказательством
    прихода «в другой раз» — иначе панель гасила бы старые пропуски одним
    фактом собственного текущего статуса участника, а не отдельным визитом.
    """
    if not student_ids:
        return {}
    rows = (
        await db.execute(
            text(
                ...
                "           AND lo2.scheduled_at >= lo.scheduled_at "
                "           AND lo2.scheduled_at < :before"
                "      ) "
