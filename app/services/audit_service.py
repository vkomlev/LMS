...
# tsk-592: преподаватель перехватил заявку помощи, которую уже вёл другой.
# Блокировка мягкая — «всё равно взять» разрешено, но у прежнего владельца
# после этого перестанет работать его lock_token, поэтому перехват обязан
# оставлять след: кто у кого забрал и когда.
TEACHER_HELP_REQUEST_CLAIM_TAKEN_OVER = "teacher.help_request.claim_taken_over"
...
# tsk-1109: ученик сам записался на бесплатный курс (кнопка в кабинете SPW).
# Единственный путь в `user_courses` мимо сотрудника — поэтому со следом.
STUDENT_COURSE_SELF_ENROLLED = "student.course.self_enrolled"
