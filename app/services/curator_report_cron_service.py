...
async def _recipients(db: AsyncSession) -> list[int]:
    """Кому идёт отчёт: владельцу школы и методистам.

    Днём 02.09 список был сужен до `admin` — потому что методист у нас оказался
    тем же преподавателем, и сводка «кто сколько не тронул» пришла бы к нему
    характеристикой на коллег. Решение оператора в тот же день: лечить не
    списком, а ролями — роль методиста снята у того, кому она не нужна.

    **Отсюда правило, которое переживёт этот код: роль методиста в этой школе
    означает доступ к работе кураторов.** Выдавая её действующему
    преподавателю, вы даёте ему цифры по коллегам. Гейт ручки
    (`app/api/v1/curator.py`) обязан совпадать с этим списком.

    Куратор про себя узнаёт отдельно: молчащему уходит персональный сигнал
    (`_nudge_silent_curators`), в котором чужих чисел нет.
    """
