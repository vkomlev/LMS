...
async def get_occurrence_summary(
    ...
    *,
    ...
    # tsk-917 п.6 (+ фикс 14.09, + tsk-976: no_show): `declined` (отказался
    # заранее), `rescheduled` (перенесён на другое occurrence) и `no_show`
    # (отмечен отсутствующим — авто по порогу или преподаватель вручную)
    # убираем из итогов ВСЕГДА, независимо от времени. Это статусы «его нет»:
    # занятие ещё не началось, идёт или уже кончилось — такой участник
    # реальную явку по ЭТОМУ occurrence уже не даст, и его «итоги» нечего
    # показывать (оператор: «если их нет, зачем видеть их итоги»). Первая
    # редакция фильтра гасила declined/rescheduled только после
    # `lesson_ends_at`, а «Подвести итоги» на панели открывается раньше — по
    # `wrapup_from` — так что оба статуса ещё протекали в сводку весь этот
    # промежуток (живой пример 14.09: occurrence 15058, сводку открыли за 7
    # минут до конца).
    #
    # `no_show` до 17.09 гасился отдельным, времязависимым гейтом (только
    # после `lesson_ends_at`) — расчёт был на то, что явка ещё может
    # измениться, пока занятие идёт (ученик подключится позже, преподаватель
    # поправит отметку вручную). На практике это давало ту же дыру, что и у
    # declined/rescheduled: живой случай 17.09, occurrence 17420, Полубенцев
    # explicit отмечен «неявка» за 6 минут до конца занятия — и всё это время
    # оставался в итогах наравне с пришедшими. Оператор решение принял
    # осознанно: риск «явка изменится позже» меньше, чем показывать явно
    # отмеченного отсутствующим ученика в списке «проверьте домашнюю работу».
    # Пришедший НЕ по записи (добавлен ad-hoc и всё равно подтвердил явку)
    # остаётся: у него та же строка со статусом `confirmed`, второго
    # признака не заводим. `scheduled` не трогаем — это сводка ДО занятия
    # (tsk-022/tsk-410), там `scheduled` — норма, а не дефект, реальная явка
    # ещё не наступила.
    participants = [
        p for p in participants
        if p.status not in ("declined", "rescheduled", "no_show")
    ]
