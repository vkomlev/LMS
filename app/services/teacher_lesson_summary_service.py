...
async def get_occurrence_summary(
    ...
    *,
    ...
    # tsk-917 п.6 (+ фикс 14.09): `declined` (отказался заранее) и
    # `rescheduled` (перенесён на другое occurrence) убираем из итогов ВСЕГДА,
    # независимо от времени. Это решённые заранее статусы: занятие ещё не
    # началось, идёт или уже кончилось — такой участник в ЭТОМ occurrence не
    # появится, и его «итоги» нечего показывать (оператор: «если их нет,
    # зачем видеть их итоги»). Первая редакция фильтра гасила declined/
    # rescheduled только после `lesson_ends_at`, а «Подвести итоги» на панели
    # открывается раньше — по `wrapup_from` — так что оба статуса ещё
    # протекали в сводку весь этот промежуток (живой пример 14.09: occurrence
    # 15058, сводку открыли за 7 минут до конца).
    participants = [p for p in participants if p.status not in ("declined", "rescheduled")]

    # `no_show` (не пришёл — авто по порогу или преподаватель отметил
    # вручную) — статус, который может ЕЩЁ поменяться, пока занятие идёт
    # (пришёл позже, преподаватель поправил отметку), поэтому гасим его
    # только когда занятие уже ЗАВЕРШИЛОСЬ и явка точно не изменится.
    ...
    # признака не заводим. Пока занятие идёт или ещё не началось —
    # `scheduled`/`no_show` не трогаем: это сводка ДО занятия (tsk-022/
    # tsk-410), там `scheduled` — норма, а не дефект, реальная явка ещё не
    # наступила.
