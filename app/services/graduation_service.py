...
Отсюда последовательность: сначала свод оплаты, потом заморозка долга, затем
закрытие персональной цены (tsk-866) и только в конце снятие с расписания.
...
class GraduationResult:
    ...
    #: Персональных цен закрыто днём ухода (tsk-866).
    closed_price_overrides: int = 0
...
async def _notify_marketers(
    ...
    *,
    ...
    Уходящего не тревожим: письмо ему — отдельное решение человека, и шлётся
    оно кнопкой из списка «Долги ушедших» (tsk-805). Раньше здесь стояло, что
    письмо уйдёт само механизмом tsk-010, — неправда: рассылка не видела
    закрытых месяцев, и до появления списка долг ушедшего держала одна эта
    запись в ленте уведомлений.
    """
    ids = [
        int(r.user_id)
        for r in (
...
async def apply(
    ...
    *,
    ...
    закрытие персональной цены → снятие с расписания. Обратный порядок стирает
    долг вместе со строкой месяца.

    **Персональная цена закрывается днём ухода (tsk-866)**, а не остаётся жить.
    Она привязана к обучению и без него теряет основание: обе цены, заведённые
    31.08.2026 как временная фиксация одного месяца, пережили выпуск и начислили
    двум ушедшим ученикам сентябрь без единого занятия — 2 750 ₽ и 5 500 ₽.
    Условие снятия было записано в примечании словами, и снять цену было некому.

    Закрытие идёт ПОСЛЕ заморозки и не переписывает уже названную сумму: цена
    действует по месяц ухода включительно (`charge_service._base_price_minor`).
    Меняется следующий месяц — тот, за который школа уже не учит.
    ...
    closed_prices = await charge_service.close_price_overrides(
        db, student_id=student_id, ends_on=today or date.today()
    )
    ...
    logger.info(
        ...
        "месяцев заморожено %s, персональных цен закрыто %s, долг %s коп.",
        student_id, slots, lessons, frozen, closed_prices, debt.due_minor,
    )
    return GraduationResult(
        ...
        closed_price_overrides=closed_prices,
    )
