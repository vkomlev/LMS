...
async def issue(
    ...
    *,
    ...
    # Дни — до БЛИЖАЙШЕГО целого, не отбрасыванием: выдача идёт через час
    # после начала занятия (отметка явки), а срок — начало следующего, и
    # «5 дней 23 часа» с отбрасыванием давало 5 вместо 6. На проде 17.09 у
    # Тихомировой окно чт→ср получило долю 0.8 вместо 1.0 (tsk-984).
    days = max(int(round((due_at - moment).total_seconds() / 86400)), 1)
