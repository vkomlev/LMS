...
- **Выполнение НЕ хранится.** Считается у источника: задание — верная сдача
  (не ручной зачёт), материал — отметка прохождения. Ученик работает обычным
...
async def issue(
    ...
    *,
    ...
    if volume <= 0:
        raise ValueError(
            "Задавать нечего: в программе ученика не осталось незавершённых элементов."
        )
