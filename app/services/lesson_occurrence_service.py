...
async def record_teacher_attendance(
    ...
    *,
    ...
    #
    # tsk-1029: выдавать только когда занятие УЖЕ ЗАКОНЧИЛОСЬ. Преподаватель
    # отмечает «Пришёл» за пару минут до урока — и в ту же секунду текущее ДЗ
    # (срок — начало этого самого урока) гасилось новым, к следующему занятию:
    # ученица сделала 17 из 17, а сводка урока показывала «0 из 34» (Киселева,
    # 21.09; у неё так все четыре выдачи подряд). Отметка до конца занятия —
    # это явка, а не повод для домашней работы; её выдаст обработчик по
    # окончании урока (`_issue_homework_after_lessons`), как и при явке,
    # отмеченной самим учеником. Отметка задним числом, когда урок давно
    # прошёл, выдаёт сразу — обработчик так далеко назад не смотрит.
    lesson_ended_at = occurrence.scheduled_at + timedelta(
        minutes=occurrence.duration_minutes
    )
    if new_status == "confirmed" and lesson_ended_at <= datetime.now(timezone.utc):
        try:
