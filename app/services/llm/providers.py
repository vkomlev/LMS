...
# Порядок изменён 2026-08-25 по решению оператора (tsk-671): `grok` переставлен
# в конец. Стенд 2026-08-06 ставил его первым по КАЧЕСТВУ, и по качеству он там
# и остаётся — но с 22.08 провайдер массово отдаёт по нему отказы
# (`no_available_provider`, «temporarily unavailable»), а перебор цепочки сейчас
# выключен из-за зависания второго запроса. То есть первая модель цепочки — не
# «лучшая», а «та, на которой ученик не упрётся в отказ». Вернуть прежний порядок
# после tsk-671 и восстановления модели у провайдера.
