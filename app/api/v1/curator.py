...
#: Отчёт — владельцу школы и методисту.
#:
#: Днём 02.09 гейт был сужен до `admin`: методист у нас оказался тем же
#: преподавателем (Серебрякова: `methodist` + `teacher`), и сводка «кто сколько
#: не тронул» стала бы для него характеристикой на коллег. Решение оператора
#: в тот же день — лечить не гейтом, а ролями: роль методиста снята у того, кому
#: она была не нужна. Методист как РОЛЬ отчёт видеть должен: неразобранные
#: сигналы — его работа тоже.
#:
#: Правило, которое отсюда следует: **роль методиста в этой школе означает
#: доступ к работе кураторов.** Выдавая её преподавателю, помнить об этом.
#: Тот же список получателей у рассылки — `curator_report_cron_service`.
