...
async def set_browser_timezone(
    ...
    """Принять системный пояс устройства и заполнить им ПУСТОЙ пояс профиля.
    ...
    tsk-753: записанный пояс автозахват больше НЕ меняет — ни свой прежний
    (`auto`), ни тем более выбранный человеком (`manual`). Раньше значение
    переписывалось при входе с любого другого устройства и об этом никто не
    узнавал: у преподавателя так оказался Екатеринбург вместо Красноярска, и
    подсказка «(у вас 16:00)» рядом с занятием выглядела утверждением
    платформы. Что делать с расхождением, решает человек в кабинете; его ответ
    приходит обычным `PATCH /me` и становится `manual`.

    Технические имена (`Etc/GMT-3`, `UTC`) отклоняются схемой с 422: в POSIX у
    них обратный знак, и как «пояс человека» они читаются наоборот.

    Ответ: `applied=false` — пояс не тронут (уже был), `applied=true` — записан.
