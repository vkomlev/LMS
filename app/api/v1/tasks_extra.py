...
# tsk-493: `/tasks/search` объявлен ДО `/tasks/{task_id}`. FastAPI матчит по
# порядку регистрации, а `{task_id}` съедает и слова тоже — типизация в
...
async def reorder_course_tasks(
    ...
    Список может быть частичным: переданные задания встают на указанные места,
    **непереданные заполняют оставшиеся, сохраняя взаимный порядок** (tsk-813).
    Итог всегда плотный ``1..N`` без дублей — поэтому в ответе приходит весь
    порядок курса, а не только переданные задания.
...
@router.post(
    ...
    description=(
        ...
        "SA (Short Answer), SA_COM (Short Answer with Comments), "
        "TBL_COM (Table answer with Comments), TA (Text Answer).\n\n"
        "**Процесс импорта:**\n"
        ...
        "- Тип `TBL_COM` (табличный ответ) генератор ставит ЯВНО — по виду ответа он не "
        "выводится: «два числа через пробел» бывает и обычным SA_COM (вывод программы в "
        "одну строку). В `correct_answer` таблица пишется значениями через пробельные "
        "символы (ячейки ряда — пробел, ряды — перевод строки), раскладка столбцов — "
        "через `task_content_json`: `{\"table\": {\"columns\": 2}}`\n\n"
        "**Обработка ошибок:**\n"
