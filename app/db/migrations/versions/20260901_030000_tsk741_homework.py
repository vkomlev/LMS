...
выводится из того же источника, где живёт прогресс ученика: `task_results`
(есть верный результат) и `student_material_progress`. Своя колонка была бы
вторым источником правды: ученик решает задание обычным путём из кабинета, а
не «внутри ДЗ», и хранимая отметка разъехалась бы с фактом в первый же день —
ровно так уже ошибались с производными величинами
(`derived_value_needs_recalc_at_source`).

**Ручной зачёт преподавателя пункт ДЗ ЗАКРЫВАЕТ, а в темп НЕ идёт** — и это
не противоречие, а два разных вопроса. «Сделано ли задание» решает
преподаватель: зачёл — значит закрыто, иначе он видел бы красную отметку,
которую сам же и снял. «С какой скоростью человек работает» — вопрос про его
собственные сдачи, и там ручные зачёты отсечены (`homework_volume_service`,
правило из `learning_gaps_service`): их ставят пачками, и норма выросла бы до
недостижимой.
