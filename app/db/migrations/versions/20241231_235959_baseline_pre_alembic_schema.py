...
2. `alembic downgrade base` на этой копии — прогоняет ВСЕ настоящие
   `downgrade()` всех 33 прежних миграций в обратном порядке до полного нуля.
...
Полный чистый прогон `alembic upgrade head` с нуля (все 34 миграции подряд,
включая эту) на пустой тестовой БД подтверждён рабочим после этой правки.
...
def upgrade() -> None:
    ...
    op.execute("""
        CREATE TABLE public.courses (
        	...
        	CONSTRAINT courses_course_uid_key UNIQUE (course_uid)
        );
    """)
    op.execute("""
    ...
    op.execute("""
        CREATE TABLE public.difficulties (
        	...
        	CONSTRAINT difficulties_code_key UNIQUE (code)
        );
    """)
    op.execute("""
    ...
    op.execute("""
        CREATE TABLE public.tasks (
        	...
        	CONSTRAINT tasks_external_uid_key UNIQUE (external_uid),
