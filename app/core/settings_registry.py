...
SETTINGS: Tuple[SettingDef, ...] = (
    ...
    SettingDef(
        key="homework_program_planned_minutes",
        group=GROUP_LESSONS,
        title="На какой недельный объём работы рассчитывать программу, минут",
        description=(
            "То же ожидание школы, что в настройке выше, но в МИНУТАХ РАБОТЫ — "
            "именно ими считается объём программы (tsk-867). Элементы "
            "разновесные: двадцать заданий с выбором ответа занимают четыре "
            "минуты, двадцать задач с решением — семьдесят девять. Умолчание "
            "90 — перевод прежних 25 элементов по измеренному среднему весу "
            "(3.7 минуты на элемент программы). У того, кто работает больше, "
            "план строится по его собственному темпу."
        ),
        kind="int",
        default=90,
        env_var=None,
    ...
    SettingDef(
        ...
        description=(
            ...
            "если он опередил начало не больше чем на этот срок. Только явка: "
            "сданное до звонка всё равно считается домашней работой."
        ),
        kind="int",
        default=5,
    ...
    SettingDef(
        key="criteria_review_enabled",
        group=GROUP_SENSORS,
        title="Разбирать короткие ответы по критериям задания",
        description=(
            "После сдачи ответа на задание, у которого нет эталона, но есть "
            "подтверждённые критерии оценивания, модель раскладывает ответ по "
            "пунктам и предлагает зачёт или незачёт. Ученик разбор не видит; "
            "решение по-прежнему принимает преподаватель в карточке проверки."
        ),
        kind="bool",
        # Выключено по умолчанию намеренно (tsk-958): включать только после
        # калибровки на живых сдачах и по решению оператора. Включение — один
        # переключатель, без выката.
        default=False,
        env_var="CRITERIA_REVIEW_ENABLED",
        warning=(
            "Включено — каждая сдача по таким заданиям стоит один вызов модели "
            "(строка `criteria_review` в отчёте о расходе). Выключено — "
            "преподаватель проверяет без подсказки, как раньше."
        ),
    ),
    SettingDef(
