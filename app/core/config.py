...
class Settings:
    def __init__(self):
        ...
        #
        # Срок обязан быть БОЛЬШЕ самого долгого прохода, иначе он делает ровно
        # то, от чего защищает: работа ещё разбирается, пометка уже протухла,
        # следующий тик берёт её второй раз и провайдеру платят дважды.
        #
        # Арифметика (tsk-678): одна работа при молчащем провайдере стоит теперь
        # не больше потолка вызова — 90 c (`Budget.BATCH.total_timeout`), пачка
        # по умолчанию десять работ, итого 15 минут в худшем случае. Прежние 15
        # минут были посчитаны по старому бюджету (60 c плюс повтор ≈ 2 минуты
        # на работу) и с включённым перебором цепочки (tsk-671) стали ровно
        # впритык. Тридцать — тот же расчёт с запасом вдвое.
