...
class ShortAnswerRules(BaseModel):
    ...
    normalization: List[NormalizationStep] = Field(
        ...
        description=(
            ...
            "Текстовые шаги: trim, lower, strip_brackets_commas, strip_punctuation, "
            "collapse_spaces — применяются в фиксированном порядке: "
            "trim → lower → strip_brackets_commas → strip_punctuation → collapse_spaces. "
            ...
            "Шаг strip_brackets_commas (tsk-977) — для заданий, где ученик пишет "
            "программу и печатает `print(list)` вместо требуемых условием чисел через "
            "пробел (`[0, 1, 0]` вместо `0 1 0`): убирает ТОЛЬКО `[`, `]`, `,`, минус и "
            "прочую пунктуацию не трогает. Подключать ТОЛЬКО заданиям, чей эталон целиком "
            "состоит из цифр/минуса/пробелов/переводов строк (`^[-0-9 \\n]+$`) — два "
            "класса заданий исключены НАВСЕГДА, оба подтверждены боевым прогоном (tsk-977): "
            "(1) эталон сам содержит `[`/`]` («Создайте список...», эталон буквально "
            "`[2, 3, 5, 7]`) — шаг сотрёт часть проверяемого синтаксиса и даст ложный "
            "зачёт (боевой пример: задание с эталоном `[4, 8]`, ответ `4,8` ложно "
            "зачёлся бы); (2) эталон — дробное число с точкой (`2.5`, `62.11`) при "
            "включённом strip_punctuation — тот шаг нарочно СКЛЕИВАЕТ цифры вокруг "
            "запятой/точки внутри числа (десятичный разделитель), а strip_brackets_commas "
            "успевает превратить запятую в пробел РАНЬШЕ, разрывая число: реальный ответ "
            "«62,11» (запятая как русский десятичный разделитель) перестаёт засчитываться "
            "против эталона «62.11» — 5 боевых регрессий на прогоне подтвердили именно "
            "этот класс, случайных не было. "
        ...
        examples=[
            ...
            ["trim", "lower", "strip_brackets_commas", "collapse_spaces"],
        ],
    )
