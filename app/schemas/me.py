...
class MeResponse(BaseModel):
    ...
    # tsk-741: человеку ещё не задавали вопрос про класс (или он на него не
    # ответил и не отказался). Тем же способом и по той же причине, что два
    # флага выше: вопрос виден на любом экране кабинета, а не в профиле, куда
    # ученик не заходит — ровно поэтому класс и был известен у 5 человек из 82.
    # Гасится ответом (school_grade заполнен либо категория оказалась не
    # школьной) или отказом (POST /me/school-grade/decline).
    school_grade_pending: bool = False
...
# ── tsk-741: вопрос про класс ────────────────────────────────────────────────

class SchoolGradeDeclineRequest(BaseModel):
    """Тело POST /me/school-grade/decline — почему человек не ответил.

    Причина нужна оператору: тех, кто промолчал, он добирает лично, а тому,
    кто сказал «я не школьник», задавать вопрос про класс больше незачем.
    Хранится в журнале событий, а не в профиле: писать «взрослый» в категорию
    по такому ответу нельзя — среди не-школьников есть и студенты, и
    абитуриенты, и правильное значение человек выберет сам в профиле.
    """

    reason: Literal["later", "not_school"] = Field(
        default="later",
        description=(
            "later — «не сейчас» (промолчал, оператор добирает лично); "
            "not_school — «я не школьник» (вопрос неприменим)."
        ),
    )


class SchoolGradeDeclineResponse(BaseModel):
    """Ответ POST /me/school-grade/decline — состояние вопроса после отказа.

    Отдельный эндпоинт, а не поле в PATCH /me: отказ — это не значение
    профиля, а решение «меня об этом не спрашивайте». Возвращает тот же флаг,
    что и `/me`, чтобы клиент погасил вопрос без второго запроса.
    """

    school_grade_pending: bool = Field(
        description="Показывать ли вопрос дальше. После отказа всегда False."
    )
