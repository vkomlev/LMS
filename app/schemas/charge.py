...
class ChargeRead(BaseModel):
    ...
    #: Занятий, которые сетка месяца предполагала, а их не было — за них не
    #: берут денег (tsk-756). Вычитается наравне с перерывом и «до прихода».
    missing_lessons: int = 0
    #: Занятий месяца на днях ПОСЛЕ ухода ученика — зеркало «до прихода»
    #: (tsk-804). Вычитается наравне с остальными тремя.
