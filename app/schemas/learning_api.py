...
class TaskStateResponse(BaseModel):
    ...
    has_io_tests: bool = Field(
        default=False,
        description=(
            "Задание проверяется исполнением программы ученика на тестах ввода/вывода "
            "(solution_rules.io_tests, tsk-953). Клиент показывает редактор кода как "
            "основное поле ответа и отправляет программу в response.value; "
            "комментарий и файл необязательны. Default false."
        ),
    )
