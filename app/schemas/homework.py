...
class HomeworkVolumeRead(BaseModel):
    ...
    catch_up_factor: float = Field(
        description=(
            ...
            "tsk-914: производная от catch_up_minutes"
        )
    )
    missed_unpaid: int = Field(
        default=0,
        description=(
            "tsk-914: пропусков, которых не хватает до плана недели по "
            "расписанию — только за них и нагоняем. Погашенные не в счёт: "
            "пришёл на другой час, работал в окне своего без отметки, "
            "переехал в другой слот"
        ),
    )
    lesson_norm_minutes: Optional[int] = Field(
        default=None,
        description=(
            "tsk-914: вес одного часа занятия для этого ученика в минутах "
            "работы — столько добавляется за каждый непогашенный пропуск"
        ),
    )
    catch_up_minutes: int = Field(
        default=0,
        description="tsk-914: сколько минут добавлено к недельной норме за пропуски",
    )
