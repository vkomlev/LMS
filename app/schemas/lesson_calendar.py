...
class TeacherSummaryCourseProgress(BaseModel):
    ...
    is_service: bool = Field(
        default=False,
        description=(
            "tsk-893: служебный курс (tsk-877) — объясняет устройство сервиса "
            "и экзамена, а не учит предмету. Клиент показывает такие отдельно "
            "и коротко: на уроке важно, пройдены ли они вообще, а не какой "
            "раздел идёт следующим"
        ),
    )
...
class TeacherSummaryReadyForHarder(BaseModel):
    """tsk-649: ученику пора дать сложнее.

    Обратная сторона `attention`: слабого видно по незачётам и заявкам помощи,
    а сильный не жалуется — он молча скучает. Признак не про скорость (два
    скоростных кандидата опровергнуты на боевых данных, tsk-589/tsk-646): он
    считает долю решённого С ПЕРВОЙ ПОПЫТКИ на НЕлёгких заданиях плюс
    отсутствие обращений за помощью на них же.

    Не означает «работа честная»: списывание у ИИ выглядит так же (tsk-646).
    Рычаг под сигнал уже есть — выборка заданий по сложности на подкурс
    (tsk-314/tsk-553), дёргает его методист.
    """

    tasks: int = Field(description="Нелёгких заданий решено за окно (первая сдача по каждому)")
    first_try_ok: int = Field(description="Из них верно с первой попытки")
    percent: int = Field(description="Доля верных с первой попытки, %")
    hard_tasks: int = Field(
        description=(
            "Сколько из заданий были уровня «Сложно»/«Проект». Часто 0–3: "
            "сложные вынесены в опциональный подкурс (tsk-347) и до них "
            "доходят единицы — по ним одним признак построить нельзя"
        )
    )
    window_days: int = Field(description="Окно наблюдения в днях")
    detail: str = Field(
        description="Готовая подпись под именем — факт, а не призыв (как у `attention`)"
    )
...
class TeacherSummaryParticipant(BaseModel):
    ...
    ready_for_harder: Optional[TeacherSummaryReadyForHarder] = Field(
        default=None,
        description=(
