...
main() {
  ...
  echo "== git fetch =="
  # Версия протокола задана ЯВНО и равна НУЛЮ (02.09.2026). Умолчание git 2.34 —
  # версия 2, и с ней этот сервер перестал ходить к GitHub посреди рабочего дня,
  # после четырёх успешных выкатов подряд:
  #
  #   без флага          — 0 успехов из 10
  #   protocol.version=2 — 0 из 5
  #   protocol.version=0 — 5 из 5
  #
  # Отказ маскируется под нехватку прав: «could not read Username for
  # https://github.com» + «expected flush after ref listing». Репозиторий при
  # этом публичный — анонимный curl на тот же адрес отдаёт 200, и никаких
  # учётных данных здесь никогда не было.
  #
  ...
  # имя пользователя — то есть деплой кажется зависшим, а не сломанным.
  #
  # Числа выше — замер, а не догадка: первая попытка объяснить это одним удачным
  # прогоном v2 дала неверный вывод. Прежде чем менять версию обратно, повторить
  # замер, а не полагаться на один успешный вызов.
  #
  # Обход здесь, а не в конфиге сервера, намеренно: конфиг потеряется при
  # пересоздании машины.
