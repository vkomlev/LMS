...
main() {
  ...
  echo "== git fetch =="
  # ВАЖНО: `origin` обязан быть SSH-адресом (`git@github.com:vkomlev/LMS.git`),
  # а не HTTPS. Проверка ниже это подтверждает и не даёт молча вернуться к
  # анонимному HTTPS при пересоздании машины.
  #
  # Почему (02.09.2026). Раньше здесь был HTTPS, и он ходил АНОНИМНО: учётных
  # данных на сервере нет вовсе, репозиторий публичный. Посреди рабочего дня,
  # после четырёх успешных выкатов подряд, fetch начал отказывать — и отказ
  # маскируется под нехватку прав: «could not read Username for
  # https://github.com» + «expected flush after ref listing».
  #
  # Коварнее всего то, КАК это выглядит: без терминала выкат падает сразу, а с
  # терминалом (`ssh -tt`, как в локальном скрипте) ВИСИТ на приглашении ввести
  # имя пользователя. Деплой кажется зависшим, а не сломанным, и время уходит
  # на поиск несуществующей проблемы в приложении.
  #
  # Первое объяснение («git 2.34 не договаривается о версии протокола») было
  # НЕВЕРНЫМ, и вот на чём оно рассыпалось: замер дал `protocol.version=0` —
  # 5 успехов из 5, но через минуту та же команда упала. Ходить анонимно
  # ненадёжно в принципе, и подбирать флаги здесь бессмысленно.
  #
  # SSH-ключ на сервере — deploy key репозитория `spw`; для ЧТЕНИЯ публичного
  # LMS этого достаточно (проверено). Станет LMS приватным — понадобится свой
  # deploy key, и падать будет уже с внятным «Permission denied», а не с
  # требованием логина.
  if ! git remote get-url origin | grep -q '^git@github.com:'; then
    echo "ОШИБКА: origin не SSH ($(git remote get-url origin))." >&2
    echo "Анонимный HTTPS к GitHub отказывает и выглядит как запрос логина." >&2
    echo "Лечение: git remote set-url origin git@github.com:vkomlev/LMS.git" >&2
    return 1
  fi
  git fetch origin
