...
## Справочники: свежий экземпляр не принимает ни одного задания (04.09.2026)

После `alembic upgrade head` база пуста **включая справочники**. Следствия:

- `difficulties` — 0 строк, а `difficulty_id` у задания обязателен → импорт
  контента падает;
- `roles` — 1 строка (`parent`, её создаёт миграция `tsk478`), остальных шести
  нет → никого нельзя сделать учеником или преподавателем.

На боевом эти строки есть, но приехали они снимком схемы, а не историей миграций
(`docs/ai/operator-runbook.md`, R-006). То есть **любой следующий экземпляр
упрётся в то же самое**.

Засев (значения совпадают с боевыми, идентификаторы раздаёт база — навязывать
боевые нельзя: на пустой базе `parent` уже занял `id=1`, а на боевом там `admin`):

```bash
sudo /opt/lms-pilot/venv/bin/python /opt/pilot-assets/import/seed_refs.py          # сухой прогон
sudo DBCHECK_OK=1 /opt/lms-pilot/venv/bin/python /opt/pilot-assets/import/seed_refs.py --apply
```

Правильное лечение — миграция, засевающая справочники; пока её нет, это ручной
шаг первичной настройки.

## Файлы курса на домене экземпляра

`/opt/pilot-assets/public/` отдаётся как `https://pilot.victor-komlev.ru/assets/`
(блок `location /assets/` в `pilot.conf`). Туда кладутся справочники и памятки,
которые нужны ученику на каждом занятии: штатная загрузка `/materials/upload`
требует сессии человека с ролью, а файлы нужны с первого дня.

Предел вложения на экземпляре — 10 МБ (`MAX_ATTACHMENT_SIZE_BYTES`). Презентация
урока 1 «Дипломата» весит 12 МБ и потому не перенесена ([[tsk-789]]).
