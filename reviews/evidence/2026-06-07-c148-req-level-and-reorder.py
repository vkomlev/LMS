# -*- coding: utf-8 -*-
"""Курс 148 (Задание 2) — нормализация req_level и порядка позиций.
...
Что делает скрипт:
  ШАГ 1: Материалы — req_level по иконкам навигатора (5 расхождений)
    id=396 → recommended (Приоритет логических операций, нет иконки)
    id=399 → skippable  (Построение ТИ с помощью Excel, 🔽)
    id=400 → skippable  (Решение задания 2 с помощью Excel, 🔽)
    id=605 → recommended (Решение задания 2, нет иконки)
    id=606 → recommended (Решение задания 2 вариант 2, нет иконки)
...
Источник данных: nav_parser.py, запущенный на navigator-po-zadaniyu-2-ege
Задача: tsk-112
"""
import io, os, re, sys
import psycopg2
...
MAT_UPDATES = {
    396: 'recommended',   # Приоритет логических операций (нет иконки)
    399: 'skippable',     # Построение ТИ с помощью Excel (🔽)
    400: 'skippable',     # Решение задания 2 с помощью Excel (🔽)
    605: 'recommended',   # Решение задания 2 (нет иконки)
    606: 'recommended',   # Решение задания 2 (вариант 2) (нет иконки)
}
