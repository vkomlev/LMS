# -*- coding: utf-8 -*-
"""Курс 155 (Задание 4) — нормализация req_level и порядка позиций.

Проблема: все задания и материалы стали required=required после ROLLBACK
из-за hardcoded-проверок в предыдущих скриптах.

Что делает скрипт:
  ШАГ 1: Материалы — req_level по иконкам навигатора (1 расхождение)
    id=639 → recommended (Решение задания 4, нет иконки в навигаторе)
  ...
  ШАГ 3: Переупорядочивание
    вводные → pos 1-10 (сохраняем текущий относительный порядок)
    non-vvod active → pos 11+ ORDER BY difficulty_id ASC, order_position ASC

Примечание: sdamgia:75241/76701/76219 помечены как [WP_DIF] парсером
(diff=3 Средняя, хотя встречаются и в разделе Сложные навигатора).
Это НАМЕРЕННО: задания дублируются в навигаторе; консервативное решение — Средняя.
НЕ менять их difficulty.

MISS материал: nav parser сообщает о zadanie-4-...-fano:5 (skippable)
которого нет в LMS — оставляем без изменений (нет данных для создания).

Источник данных: nav_parser.py, запущенный на navigator-po-zadaniyu-4-ege
Задача: tsk-112
"""
import io, os, re, sys
import psycopg2
...
# Материалы: {id: нужное req_level} — из вывода nav_parser
MAT_UPDATES = {
    639: 'recommended',   # Решение задания 4 (нет иконки в навигаторе)
}
