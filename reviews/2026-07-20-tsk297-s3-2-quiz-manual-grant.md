...
## Живой прогон на проде (2026-07-20, после деплоя `dca6192`)

Под сессией аккаунта 2 (methodist/teacher), карточка ученика 142, курс 651
«Пробное занятие IT-школы (6-9 класс)», квиз 5266 (первое задание темы):

| Проверка | Результат |
|---|---|
| `POST .../progress/tasks/5266` по кнопке «Зачесть» | **422** ✔ |
| Запись после 2 нажатий (`attempts.source_system='manual_teacher'` у 142) | **0** ✔ отказ действительно ничего не пишет |
| Ручные зачёты по всем квизам прода | **0** ✔ |
| Реакция интерфейса на отказ | **ничего** ✘ см. ниже |

Побочно подтвердилась вся посылка запрета: у ученика 142 на квизе 5266 лежит
**настоящий** ответ (`source_system='spw_web'`, `scale_scores={"python":2,
"информатика":0}`). Именно эти шкалы ручной зачёт затёр бы нулями, а ученику
закрыл бы повторный ответ. Заодно проверено решение «гейт до проверки уже
PASSED»: задание у 142 уже пройдено, и старая логика вернула бы тихий
`already=true` — сейчас честный 422.

### Находка живого прогона: отказ молчит в интерфейсе (P2, SPW)

Нажатие «Зачесть» на квизе даёт 422, но **пользователь не видит ничего** —
ни сообщения, ни подсветки, кнопка просто не срабатывает. Проверено дважды,
в дереве доступности после клика нет ни alert, ни toast, ни текста ошибки.

Это правит оценку из раздела ниже: деградация до правки фронта **не «мягкая»,
а молчаливая**. Преподаватель будет жать кнопку и считать, что сломалась
система. Внятный `detail` на сервере есть — его просто никто не показывает.
Поднимает приоритет follow-up про SPW: показ ошибки нужен раньше, чем
скрытие кнопки.
...
3. **SPW** (уточнено живым прогоном): (а) **показывать ошибку API** — сейчас 422
   проглатывается молча, это первично; (б) скрыть кнопку зачёта по
   `manual_grantable`; (в) показать `skipped_quiz` в итоге массовой операции.
   Прежняя формулировка «деградация мягкая, пользователь увидит 422» **неверна** —
