...
## Осталось (не в этой правке)

- `/review-gate` перед мержем в `main`.
- Обновить cross-project зеркало контракта (`ContentBackbone/docs/cross-project/contracts/lms-api.md` + CHANGELOG): новый 403.
- Развернуть + живая проверка на проде (ветвь А): записанный ученик отвечает, посторонний получает 403.
