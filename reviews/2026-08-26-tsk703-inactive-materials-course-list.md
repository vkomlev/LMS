...
Прод, `Deployed: e49909e`, живая сессия ученика 142
(`/me` → `{"id":142,…,"roles":["parent","student"]}` — привилегий нет).
Площадка — курс **1064** «Трек 1. Создание IT-продуктов»: 64 выключенных
материала из 65.

| Запрос / экран | Ожидалось | Факт |
|---|---|---|
| `GET /courses/1064/materials?limit=500` (ученик) | 1 материал, выключенных нет | **1** (`id=2217`), выключенных **0**, `total=1` |
| `GET /courses/1064/materials?is_active=false&limit=500` (ученик) | обход закрыт | тот же **1** активный, выключенных **0** |
| `GET /courses/1064/materials?limit=500` с `X-API-Key` (на сервере) | всё как раньше | **65**, из них **64** выключенных |
| `GET /courses/138/materials?limit=500` (ученик), контроль | 9 из 12 | **9**, выключенных **0** |
| Экран программы курса ученика (`/courses/wp:ai-predprinimatel`) | вид не изменился | 288 материалов / 167 задач, разделы и прогресс на месте, названия материалов живые, **0** запасных подписей «Материал #N» / «Задание #N» |

До правки тот же запрос под учеником возвращал 65 материалов — 64 снятых с
публикации, каждый вместе с телом (`content`).
