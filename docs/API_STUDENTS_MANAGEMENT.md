...
| `403` | Forbidden | Неверный или отсутствующий API ключ |
| `409` | Conflict | Дубликат связи: студент уже привязан к курсу (tsk-574) |
...
- При попытке создать дубликат связи возвращается ошибка 409 Conflict.
  До tsk-574 (2026-08-07) этот путь отдавал 500 — повторное назначение было
  неотличимо от отказа сервера
...
|-----|----------|---------|----------------|
| `400` | Bad Request | Некорректный запрос | `{"detail": "Bad request"}` |
...
| `409` | Conflict | Конфликт состояния: связь студент-курс уже существует | `{"error": "domain_error", "detail": "Курс уже назначен этому ученику"}` |
...
- Для связи студент-курс: вернётся ошибка `409 Conflict` (tsk-574)
- Для массового зачисления (`POST /users/{id}/courses/bulk`): уже привязанные
  курсы просто пропускаются, ошибки не будет
