...
- **Либо** `difficulty_code` ИЛИ `difficulty_id` в запросе — одна сложность на весь импорт.
- **Либо** колонка в таблице:
  - **`difficulty_uid`** (рекомендуется): значения из справочника БД (например `theory`, `easy`, `normal`, `hard`, `project`). Список: GET /api/v1/meta/tasks.
  - **`difficulty_code`**: коды `EASY`, `NORMAL`, `HARD` и т.д. (регистр не важен).
  Если в строке пусто — используется сложность из запроса.
