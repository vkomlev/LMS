...
|---------|----------|------------|--------|-------------------|
| `course_uid` | Код курса для **этой строки** (courses.course_uid). Если заполнено — переопределяет `course_code/course_id` из запроса | Строка | `PY` | Когда нужен импорт в разные подкурсы в одной таблице |
| `difficulty_uid` | UID сложности для **этой строки** (difficulties.uid). Маппинг через БД: значение из справочника сложностей. Если заполнено — переопределяет сложность из запроса. Примеры: `theory`, `easy`, `normal`, `hard`, `project` (см. GET /meta/tasks) | Строка | `normal` | Рекомендуемый способ: одна колонка, значения из БД |
| `difficulty_code` | Код сложности для **этой строки** (difficulties.code). Альтернатива `difficulty_uid`. Допустимые значения: `THEORY`, `EASY`, `NORMAL`, `HARD`, `PROJECT` (регистр не важен) | Строка | `NORMAL` | Когда у заданий в одной таблице разная сложность |
...
| external_uid | course_uid | type | stem | options | correct_answer | max_score | code | title | prompt | input_link |
|--------------|------------|------|------|---------|----------------|-----------|------|-------|--------|------------|
| TASK-SC-001 | PY | SC | Что такое переменная? | A: Область памяти \| B: Функция | A | 10 | PY-VAR-001 | Переменные | Переменная хранит значение | |
| TASK-MC-001 | PY | MC | Выберите неизменяемые типы | A: list \| B: tuple \| C: str | B,C | 15 | PY-IMMUT-001 | Неизменяемые типы | | |
| TASK-SA-001 | PY | SA | Сколько байт в int? | | 8 | 10 | PY-INT-001 | Размер int | | |
...
| `course_uid`, `course_code`, `course`, `курс` | `course_uid` |
| `difficulty_uid`, `difficulty uid` | `difficulty_uid` (маппинг через БД) |
| `difficulty_code`, `difficulty`, `сложность` | `difficulty_code` |
...
2. **Название листа:**
   - По умолчанию используется `GSHEETS_WORKSHEET_NAME` (если не задано — **`Tasks`**)
...
```typescript
{
  ...
  // Курс и сложность
  course_code?: string;     // Код курса (courses.course_uid) — если курс один на весь импорт
  course_id?: number;       // ID курса (если указан, используется вместо course_code)
  difficulty_code?: string; // Код сложности (difficulties.code) — если сложность одна на весь импорт
  difficulty_id?: number;   // ID сложности (если указан, используется вместо difficulty_code). Сложность обязательна: либо в запросе, либо в колонке difficulty_code в таблице
...
**Импорт в разные подкурсы (курс на строке):**

- В таблице добавьте колонку `course_uid` и заполните её для каждой строки.
- В запросе можно **не** указывать `course_code/course_id` (если `course_uid` заполнен в каждой строке).

**Разная сложность заданий (сложность на строке):**

- В таблице добавьте колонку **`difficulty_uid`** (рекомендуется) или `difficulty_code`. Значения `difficulty_uid` берутся из справочника БД (GET /api/v1/meta/tasks): `theory`, `easy`, `normal`, `hard`, `project`. Если в строке ячейка пустая — используется сложность из запроса.
- В запросе можно **не** указывать `difficulty_code`/`difficulty_id`, если колонка присутствует и заполнена во всех строках.
