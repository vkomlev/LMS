...
- **tsk-429** (Календарь LMS Фаза 2) — явка ученика (`POST /lesson-occurrences/{id}/attendance`, `GET /me/lesson-occurrences`) + APScheduler-тик reminder/авто-no_show (`app/services/lesson_attendance_cron_service.py`). Первая фаза с реальным пользовательским эффектом. Панель преподавателя — Фаза 3, ещё не реализована; она разблокирует tsk-410.
