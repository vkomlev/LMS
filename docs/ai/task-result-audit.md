# task_result_audit — журнал правок оценок (`task_results.score` / `is_correct`)

**Источники:** таблица `public.task_result_audit` (append-only через триггер `task_result_audit_no_modify`), триггер `trg_task_result_audit_update` на `task_results` (функция `log_task_result_audit`), модель `app/models/task_result_audit.py`.
**Миграция:** `2026_09_07_tsk803_task_result_audit.py`.
...
**Задача:** tsk-803. Заведена по следам tsk-801: правка боевых оценок 17 учеников (51 ложный незачёт) оказалась откатываемой только по бэкапу в скратчпаде сессии и списку id, вписанному в задачу руками.
...
## Границы: что журнал НЕ покрывает

- **INSERT не аудируется.** Сама запись результата и есть его первое состояние, а поток вставок на порядок больше: на момент миграции прод показывал 22 122 вставки против 4 944 обновлений. Аудит на INSERT обогнал бы исходную таблицу за неделю, ничего не добавив.
- **DELETE не аудируется.** Осознанная граница tsk-803: удаление результата (39 за тот же период, почти все каскадом от `users`/`tasks`) следа не оставляет. Значит, пара «DELETE + INSERT» остаётся способом заменить оценку неотслеживаемо. Если случай возникнет — включается той же функцией и вторым триггером, колонка `action` под это зарезервирована.
...
| `action` | varchar(16) NOT NULL | Сейчас всегда `'UPDATE'`; `'DELETE'` зарезервирован (CHECK допускает оба). |
| `old_score` / `new_score` | integer NULL | Балл до и после. |
...
-- ожидается trg_task_result_audit_update со статусом 'O' (включён)
```
