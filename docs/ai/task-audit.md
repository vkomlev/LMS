...
### Нормализация схемой правкой не считается

Импорт всегда пишет ПРОВАЛИДИРОВАННОЕ значение (`model_dump()`), а в базе может
лежать более раннее — без полей, которые схема завела позже (`scales`, `table`,
`multiline_answer` в условии; `quiz`, `turtle_sim`, `requires_attachment`,
`partial_auto_check`, `grading_criteria` в правиле). Буквальное сравнение jsonb
называло это изменением: повторный импорт двигал `updated_at`, писал пустую
строку в журнал, а переиздание курса из ContentBackbone потом считало такое
задание правленным вручную и переставало его обновлять.

Поэтому `bulk_upsert` сравнивает обе стороны в одном виде
(`_same_after_validation`): сохранённое значение прогоняется через ту же схему,
и при совпадении поле не переписывается вовсе — в базе остаётся ровно то, что
было. Следствия:

- повторное переиздание без правок не оставляет следов ни в `updated_at`, ни в журнале;
- исторические значения так и остаются в исходной форме — «донормализация» и была
  тем, что двигало отметку;
- значение, которое НЕ проходит нынешнюю схему (исторически битое), переписывается:
  такое как раз надо починить.
