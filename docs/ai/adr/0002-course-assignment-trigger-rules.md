...
- **Статус:** Accepted, реализовано (фундамент + ручное назначение + UI-кнопка, 2026-07-25)
- **Дата:** 2026-06-24
...
### 6. Поиск курса для UI-селектора (2026-07-25)
...
- **Follow-up:** ~~UI-кнопка в SPW/teacher API~~ (закрыто 2026-07-25, tsk-031 хвост —
  `GET /teacher/courses/search` + SPW `CourseAssignButton`); админка правил; перенос
  оценки в асинхронный воркер; правило `course_failed` по накопленной истории (не только
  в рамках одной попытки).
