...
- Следствие: **два полных прогона можно гнать параллельно** — правило ADR-0008
  ...
  по 22 минуты каждый.
- **Redis тоже свой на прогон (tsk-880).** `tests/run_redis.py` занимает
  свободную логическую базу (у сервера их 16), подменяет `REDIS_URL` до импорта
  приложения — `get_redis` кеширует пул на первый URL — и очищает базу в конце.
  Захват атомарный (`SET NX` ключа аренды со сроком 2 часа), поэтому
  одновременно стартующие прогоны получают разные базы. Без этого прогоны
  делили ограничители частоты, а те ключуются по IP тестового клиента, общему
  для всех (`ml_send:127.0.0.1` — 5 запросов за 10 минут, `quiz_lead` — 10 за
  час). База из `.env` (обычно 2) не занимается и не чистится вовсе: там живёт
  FSM бота разработки. Управление: `LMS_TEST_REDIS_ISOLATION=auto|on|off`.
- Чего изоляция НЕ закрывает, кроме времени: **каталог загрузок остаётся общим
  на рабочее дерево**. Тест не должен опираться на состояние каталога целиком —
  только на свои файлы: имя чека начинается с id ученика, имя вложения попытки
  — с id попытки, поэтому фильтр `glob(f"{id}_*")` и адресация конкретного
  файла безопасны, а `iterdir()` по всему каталогу — нет (tsk-878: чужой файл
  соседнего прогона уронил проверку «после отказа чек не сохранился»).
