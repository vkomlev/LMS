...
**Присоединение к существующему занятию вместо создания дубля (tsk-443,
продолжение №2 — реальный баг, найден по факту дубля в проде).** Свободный
ввод даты/времени в `BookLessonSection` создавал через
`POST /lesson-occurrences/ad-hoc` НОВОЕ occurrence, даже когда время
совпадало с уже существующим (реальный инцидент: occurrence id=460,
`slot_id=NULL`, 1 участник Денис Ильин — точный дубль id=23, `slot_id=12`,
та же минута, 3 участника, тот же преподаватель). Оператор: "Он не должен
гадать... должен выбрать из ближайших доступных слотов. Под него не
делается отдельный слот, а он присоединяется к существующему."

Новое: `GET /me/lesson-occurrences/bookable?limit=` —
`lesson_occurrence_service.list_bookable_occurrences_for_student` — ближайшие
БУДУЩИЕ occurrence преподавателей ученика (`list_for_teachers`, объединение
через M2M `lesson_occurrence_teacher` + колонку, tsk-443), исключая уже
прошедшие и те, где ученик уже участник; `teacher_names` — все преподаватели
occurrence (`list_teacher_names_for_occurrences`, для со-преподавания).
`POST /lesson-occurrences/{id}/join` —
`lesson_occurrence_service.join_occurrence_as_student` — добавляет ученика
участником УЖЕ существующего occurrence (идемпотентно, 404 не найдено, 409
уже прошло / пересекается с другим активным занятием) — новый occurrence НЕ
создаётся.

SPW `BookLessonSection.tsx` полностью переписан: список ближайших bookable
occurrence (кнопки время+преподаватель) вместо datetime-local/duration/
teacher-select — клик = join. `POST /lesson-occurrences/ad-hoc` (и
`useBookAdHocLesson`) НЕ убраны — остаются для `AdHocBookingSection`
(запись на отработку после пропущенного занятия, намеренно свободный ввод
времени — отдельное решение оператора из tsk-436, тот путь не переиспользует
регулярный слот по смыслу).

Реальный дубль (id=460→23) исправлен точечным скриптом
`scripts/tsk443_fix_duplicate_occurrence_460.py`: участие и
`attendance_event` Дениса Ильина перенесены на верный occurrence (ссылка
исправлена, факт "кто/что/когда" не переписан), пустой дубль удалён.
...
## Кураторство: ответственный преподаватель у ученика (tsk-742, применено)

**Проблема.** Ученик был «ничей» между занятиями. `student_teacher_links` —
это «все преподаватели ученика», многие-ко-многим без ответственного: по ней
нельзя ответить, кто отвечает за результат, потому что отвечают все. Следствие
видно в данных: на 02.09.2026 в проде висели 17 неразобранных
`learning_gap_signal`, старший с 09.08 — адресата у сигнала не было.

**`student_curator`** (ревизия `tsk742_student_curator`) — периоды
ответственности ОТРЕЗКАМИ, а не колонкой в `users`: через полгода вопрос «кто
отвечал за ученика в сентябре» должен иметь ответ. Открытая строка
(`ended_at IS NULL`) — действующий куратор; закрытые — история. Единственность
действующего держит частичный уникальный индекс
`uq_student_curator_current ON (student_id) WHERE ended_at IS NULL`, а не
проверка в коде: две одновременные смены иначе оставили бы двух ответственных.

Две причины и оба смысла нужны: `reason` — почему назначен ЭТОТ куратор,
`ended_reason` — почему прежний перестал им быть.

**`student_teacher_links` НЕ тронута и не заменяется.** На ней держится ACL
кабинета преподавателя (лента активности, очередь проверок, заявки помощи,
правка прогресса, сообщения). Сузить её до одного человека значило бы отобрать
доступ у тех, кто с учеником реально работает.

**Раскладка выводится из расписания, а не назначается** — правило целиком
словами в [docs/curator-charter.md](../curator-charter.md) § 6, исполнение в
`app/services/curator_service.py`. Ведущий занятия ищется В ДВУХ МЕСТАХ:
колонка (`lesson_slot.teacher_id`, `lesson_occurrence.teacher_id`) и таблицы
совместного ведения (`lesson_slot_teacher`, `lesson_occurrence_teacher`) —
отбор по одной пропускает занятия, заведённые другим способом. Кандидаты —
роль `teacher` минус носители роли `admin`: владелец школы из кураторства вышел.

**При слиянии учёток кураторство переносится** (`user_merge_service.
_move_curator_periods`) — отдельным шагом, а не через `CONFLICT_MOVES`: там
разводят по составной уникальности, а здесь она частичная и одноколоночная.
Оставшись у слитой учётки, закрепление молча пропадает, и живой ученик снова
становится «ничей» — тот же класс, что перерыв (tsk-610) и ручная цена
(tsk-548).
