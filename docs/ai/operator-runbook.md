...
## R-010: Копия `.env` перед правкой — убирать сразу после проверки

**Симптом:** в `/opt/lms` накапливаются файлы `.env.bak-tsk<NNN>`. Каждый —
полный снимок боевых секретов на диске (DSN базы, ключи подписи сессий,
`FERNET_MASTER_KEY`, ключи ЮKassa, сервисные `VALID_API_KEYS`). К 2026-08-02
их набралось четыре, старейшему почти месяц; отличались они только историей
ротации `VALID_API_KEYS` — то есть хранили уже недействующие ключи.

**Почему это плохо:** снимок живёт под теми же правами `600 app`, но переживает
ротацию. Утечка одной копии выдаёт секреты той эпохи целиком, а ценности в ней
ноль — восстановить `.env` можно из `.env.example` плюс актуальных значений.

### Autonomous workaround (Claude делает сам)

Копия перед правкой — по-прежнему правильный ход, но она **временная**:

```bash
ssh lms-spw-vds "sudo -u app cp /opt/lms/.env /opt/lms/.env.bak-tskNNN"
# …правка, перезапуск, проверка /health и живой прогон…
ssh lms-spw-vds "sudo -u app rm -f /opt/lms/.env.bak-tskNNN"
```

Удаление копии — часть той же задачи, а не «потом». Если задача закрыта, а
файл остался, его надо убрать при следующей же работе на сервере.

Перед удалением чужих или старых копий — сверить, не осталось ли в них
уникальных значений (скрипт `scratchpad`-уровня: разобрать оба файла в словари,
сравнить имена переменных и отпечатки `sha256[:8]` значений; **сами значения не
печатать**). Признак безопасного удаления: нет переменных, которых нет в
текущем `.env`, а расхождения объясняются ротацией.

### Operator instruction

Не требуется — действие полностью автономно (файлы на своём сервере, доступ по
ssh есть, значения секретов не нужны).

## R-011: Что сделать руками в панели Timeweb после инцидента 17.08.2026 (tsk-621)

**Симптом:** прод волнами отдавал 500 (`QueuePool limit of size 5 overflow 10 reached`),
об инциденте узнали от людей, а не от системы мониторинга.

### Главное: наращивать ресурсы не нужно

Причина сбоя — не нехватка мощности, а блокировка строки в базе (правка кода, tsk-621).
Замеры во время инцидента:

| Ресурс | Что было | Вывод |
|---|---|---|
| Процессор сервера приложения | простой 98–100%, средняя нагрузка 0.0 | запас огромный |
| Память сервера приложения | свободно 2.3 ГБ из 3.9 ГБ | запас есть |
| Диск | занято 13% из 49 ГБ | запас есть |
| Подключения к базе | 6 занятых из 200 разрешённых | запас 33-кратный |
| Размер базы | 152 МБ | тариф ни при чём |
| Сеть до базы | 0% потерь, задержка 0.5 мс | здорова |

**Не нужно:** повышать тариф кластера базы, добавлять реплики, увеличивать процессор или
память серверов, заказывать балансировщик. Деньги уйдут, сбои останутся.

**Не нужно:** настраивать приватную сеть — она уже есть. Кластер базы доступен внутри сети
по адресу `192.168.0.4` (проверено: тот же кластер, `system_identifier` совпадает), серверы
приложения и ботов — `192.168.0.10` и `192.168.0.8`.

### Что действительно стоит сделать руками (10–15 минут)

Всё остальное по этой задаче делается по ssh и в коде, оператор не нужен.

**Шаг 1. Подключить мониторинг доступности — самое важное.**
Сейчас его нет: о часовом сбое узнали от пользователей. Услуга стоит 30 ₽ в месяц за
проверку ([документация](https://timeweb.cloud/docs/monitoring)).

1. Панель управления → раздел **«Мониторинг»** → добавить проверку.
2. Создать две проверки:
   - `https://learn.victor-komlev.ru/` — кабинет учеников и преподавателей;
   - `https://api.learn.victor-komlev.ru/health` — сам API. Эта важнее: она ловит
     отказ раньше, чем его замечает человек. Ожидаемый ответ — код 200.
3. Подключить уведомления в Telegram (там же выбираются каналы: email, Telegram, MAX).
4. **Как понять, что получилось:** остановить проверку невозможно «понарошку», поэтому
   достаточно убедиться, что в списке проверок обе строки в состоянии «доступен», а в
   Telegram пришло подтверждение подписки.

**Шаг 2. Проверить резервные копии кластера базы.**
База — 152 МБ, копии дешёвые, но их наличие нигде не подтверждено.

1. Панель → **«Базы данных»** → кластер `learn` → вкладка **«Бэкапы»**
   ([документация](https://timeweb.cloud/docs/dbaas/dbaas-manage)).
2. Убедиться, что включены физические копии, и задать расписание: **ежедневно, хранить
   не меньше 7 копий**. Учебные данные меняются каждый день, недельная глубина позволяет
   откатиться к любому дню прошлой недели.
3. **Как понять, что получилось:** в списке копий появляется хотя бы одна с сегодняшней
   датой (первая создаётся по расписанию, не мгновенно).

**Шаг 3. Проверить, куда приходят уведомления аккаунта.**
Панель → настройки аккаунта → **«Уведомления»**
([документация](https://timeweb.cloud/docs/account-management/notifications)).
Нужны включённые уведомления о событиях серверов и баз (перезагрузка, нехватка места,
окно обслуживания кластера) на актуальный адрес и в Telegram.

**Шаг 4 (по желанию). Посмотреть графики кластера базы.**
Панель → «Базы данных» → кластер → графики нагрузки. Полезно раз в неделю глянуть число
подключений: после правок tsk-621 приложение держит до 50 подключений на процесс при
лимите 200. Если график подберётся к 150 — сказать, тогда поднимем лимит или тариф.
Сейчас это далеко за горизонтом.

### Что уже сделано без панели (для контекста)

- Обновление отметки активности сессии — не чаще раза в минуту (снята блокировка строки,
  из-за которой запросы одного пользователя выстраивались в очередь).
- Пул подключений: было 15, стало 30 (10 постоянных + 20 сверх лимита), добавлены проверка
  живости подключения и обновление раз в полчаса. Больше не ставили сознательно: лимит
  200 подключений на сервере базы общий, рядом живут три полигонных сервиса.
- В лог пишутся запросы дольше 3 секунд — иначе следующий такой разбор снова пойдёт вслепую.
