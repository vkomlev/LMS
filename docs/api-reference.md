...
### POST /attempts/{attempt_id}/attachments

Загрузить файл к ответу в рамках активной попытки.

**Правила:**
- доступ: владелец попытки или service caller;
- тело: `multipart/form-data`, поле `file`;
- одна попытка хранит одно актуальное вложение;
- повторная успешная загрузка заменяет предыдущий файл, старая ссылка после этого возвращает `404`;
- завершенная или отмененная попытка возвращает `409`;
- превышение `MAX_ATTACHMENT_SIZE_BYTES` возвращает `413`.

**Ответ (201 Created):**
```json
{
  "attachment_id": "123_abcd..._solution.py",
  "attachment_url": "/api/v1/attempts/123/attachments/123_abcd..._solution.py",
...
### GET /attempts/{attempt_id}/attachments/{attachment_id}

Скачать вложение ответа.

**Правила:**
- доступ: владелец попытки или service caller;
- `attachment_id` должен соответствовать серверному формату `<attempt_id>_<uuid32>_<safe_filename>`;
- чужой, невалидный или замененный `attachment_id` возвращает `404`.

**Ответ (200 OK):** поток файла.
...
| GET | `/teacher/help-requests?teacher_id=&status=open\|closed\|all&request_type=manual_help\|blocked_limit\|all&sort=priority\|created_at\|due_at&limit=&offset=` | Список заявок на помощь (этап 3.8/3.8.1/3.9). Поля: request_type, auto_created, context, priority, due_at, is_overdue. Фильтр request_type; sort по умолчанию priority. ACL: назначенный teacher, student_teacher_links, teacher_courses или роль methodist. |
| GET | `/teacher/help-requests/{request_id}?teacher_id=` | Карточка заявки (поля списка + message, closed_at, closed_by, resolution_comment, history, priority, due_at, is_overdue). |
| POST | `/teacher/help-requests/{request_id}/close` | Закрыть заявку (body: `closed_by`, `resolution_comment`, опц. `lock_token`). Идемпотентно. При невалидном lock_token — 409 (этап 3.9). |
| POST | `/teacher/help-requests/{request_id}/reply` | Ответить студенту (body: `teacher_id`, `message`, `close_after_reply`, `idempotency_key`, опц. `lock_token`). Создаёт сообщение в messages, опционально закрывает заявку. При невалидном lock_token — 409 (этап 3.9). |
| POST | `/teacher/help-requests/claim-next` | Взять следующий открытый help-request (этап 3.9). Body: `teacher_id`, `request_type` (manual_help\|blocked_limit\|all), `status` (open), `ttl_sec` (30..600), опц. `idempotency_key`, `course_id`. Ответ: `item`, `lock_token`, `lock_expires_at`, `empty`. |
| POST | `/teacher/help-requests/{request_id}/release` | Освободить блокировку заявки (этап 3.9). Body: `teacher_id`, `lock_token`. 409 при неверном токене. Идемпотентно: уже свободно — 200, `released: false`. |
| GET | `/teacher/workload?teacher_id=` | Сводка нагрузки преподавателя (этап 3.9): open_help_requests_total, open_blocked_limit_total, open_manual_help_total, pending_manual_reviews_total, overdue_total. |
| POST | `/teacher/reviews/claim-next` | Взять следующий результат на ручную проверку (этап 3.9). Body: `teacher_id`, `ttl_sec`, опц. `course_id`, `user_id`, `idempotency_key`. Ответ: `item`, `lock_token`, `lock_expires_at`, `empty`. |
| POST | `/teacher/reviews/{result_id}/release` | Освободить блокировку проверки (этап 3.9). Body: `teacher_id`, `lock_token`. 409 при неверном токене. Идемпотентно. |
| | | В списке/карточке help-requests с этапа 3.9: `priority`, `due_at`, `is_overdue`. Query `sort=priority\|created_at\|due_at`. Close/reply принимают опц. `lock_token`; при невалидном — 409. |

Ответ `POST /learning/tasks/{task_id}/request-help` с этапа 3.8 может содержать опциональное поле `request_id` (ID заявки в help_requests). С этапа 3.8.1 при `GET /learning/next-item` или `GET /learning/tasks/{task_id}/state` с типом/состоянием `blocked_limit`/`BLOCKED_LIMIT` заявка на помощь создаётся автоматически (request_type=blocked_limit, auto_created=true, context с attempts_used/attempts_limit_effective). Список заявок можно фильтровать по `request_type=manual_help|blocked_limit|all`. С этапа 3.9: claim-next/release для help-requests и reviews, workload, приоритет/SLA в списке. Smoke: [smoke-learning-engine-stage3-8-help-requests.md](smoke-learning-engine-stage3-8-help-requests.md), [smoke-learning-engine-stage3-9-next-modes.md](smoke-learning-engine-stage3-9-next-modes.md).
