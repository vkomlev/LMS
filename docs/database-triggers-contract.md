...
2. **При UPDATE внутри одного курса (перестановка):**
...
3. **При UPDATE со сменой `course_id` (переезд, tsk-802):**
   - Это **не** перестановка: старая позиция относится к другому курсу, и сравнивать её с новой нельзя
   - В **исходном** курсе переезд обрабатывается как удаление: задания ниже `OLD.order_position` поднимаются на единицу, дыры не остаётся
   - В **целевом** курсе — как вставка: позиция **зажимается в диапазон курса**. `NULL` либо значение больше `MAX(order_position) + 1` → задание встаёт в конец; достижимая позиция уважается, соседи сдвигаются вправо (+1)
...
#### ⚠️ Переезд задания между курсами: что важно знать вызывающему коду

Триггер **не видит списка колонок в `SET`**. `UPDATE tasks SET course_id = 111 WHERE id = …`
доезжает до него как `NEW.order_position = OLD.order_position` — унаследованная цифра из
чужого курса, неотличимая от сознательно заданной. Отсюда правило зажима, а не догадка
«совпало со старой → значит позицию не указывали»: перенос «второго задания на второе
место» — обычное дело, и его намерение сохраняется.

Практические следствия:

```sql
-- Переезд без указания места: позиция 41 недостижима в курсе из 5 заданий → задание
-- встанет 6-м. Дыр 6..40 не будет.
UPDATE tasks SET course_id = 111 WHERE id = 146;

-- Переезд с явным местом: 3 достижимо → задание встанет 3-м, соседи сдвинутся вправо.
UPDATE tasks SET course_id = 111, order_position = 3 WHERE id = 146;
```

До tsk-802 обе формы портили **чужие** задания целевого курса: сдвиг считался по диапазону
между старой (из исходного курса) и новой позицией. Инцидент tsk-801: перенос задания 146
(108/41 → 111/43) сдвинул постороннее задание 190 с 41 на 40, где уже стояло 188 — дубликат
позиции в курсе, которого никто не трогал. Ошибки при этом не было: транзакция проходила,
собственное задание оказывалось на месте, и заметить порчу можно было только сверкой
позиций всего курса. Регрессия закрыта тестами `tests/test_tsk802_task_move_between_courses.py`.
...
Допускается отправлять порядок только для подмножества заданий курса — так
устроен drag-list методиста. **Переданные задания встают на указанные места,
непереданные заполняют оставшиеся слоты по возрастанию, сохраняя взаимный
порядок** (tsk-813). Итог всегда плотный `1..N` без дублей, поэтому в ответе
приходит весь порядок курса, а не только переданные задания.

До tsk-813 непереданные просто сохраняли свои числа, а коллизии объявлялись
зоной ответственности клиента. Это и породило дубликаты позиций в курсах 146,
147 и 1397 (tsk-810): кабинет методиста нумерует от единицы **отфильтрованный**
список, и под фильтром «Активные» активные задания получали `1..N` поверх
чисел, на которых стояли выключенные. Триггер здесь не помогает — реордер
глушит его намеренно, — поэтому порядок достраивается на стороне сервиса, а
уникальность пары `(course_id, order_position)` подпёрта отложенным
ограничением БД (см. раздел 16).
