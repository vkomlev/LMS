...
## Operator handoff vs autonomous action (обязательно)

При планировании и выполнении задачи всегда классифицируй каждый шаг:

**Делай сам через скиллы/MCP/код, не дожидаясь оператора:**
- Чтение/правка файлов в проекте
- DB-операции через MCP `postgresql` (read) или скрипты с loaded `.env` (write при явной задаче)
- Запуск/остановка процессов на этой машине, чтение `logs/app.log`
- Workaround'ы: выпуск тестового токена в БД, ручная HTTP-команда вместо письма, прямой INSERT когда автоматический путь сломан внешним сервисом
- Любая read-only диагностика и smoke-тесты на localhost

**Готовь пошаговую инструкцию оператору только если:**
- Действие в внешнем сервисе с человеческим логином: создание API-ключа в Resend / VK / Telegram BotFather, верификация домена в DNS
- Действие требует ручного ввода secrets, которые я не должен видеть/писать в файлы (платёжные данные, root-пароли)
- Физический мир: подключить устройство, проверить почту глазами
- Restart процессов, которые оператор запускал вручную в своём терминале (Ctrl+C → перезапуск с обновлённым `.env`)

**Формат инструкции для оператора (когда нужна):**
1. Заголовок «Что от тебя нужно» + оценка времени
2. Пронумерованные шаги, **каждый шаг проверяемо завершается** (что я увижу как результат)
3. Готовые команды/значения, которые можно копипастить
4. Раздел «Если не сработало» — диагностика с точным грепом по логам и частыми причинами
5. Что я сделаю после, как только оператор вернётся с подтверждением

**Антипаттерн:** «Проверь X, проверь Y, попробуй ещё раз» без точных команд и ожидаемого результата. Замени на: «Выполни команду `…`, ожидай вывод вида `…`. Если вместо этого `…` — причина в `…`».

При первом столкновении с операторской зависимостью в задаче — записать workaround в `docs/ai/operator-runbook.md` (создать при необходимости), чтобы повторный сценарий шёл автономно.
