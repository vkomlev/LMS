...
$plan = @(
    ...
    # LimitMinutes 90: с tsk-852 сверка читает задания пачками (страницами банка
    # LMS) вместо запроса на каждое — полный прогон по 5089 ключам занимает около
    # минуты против прежних 70. Девяносто минут — не оценка длительности, а запас
    # на деградацию: пачка отказала, сверка ушла на запасной путь по одному
    # заданию. Сводке в 09:45 нужен готовый итог.
