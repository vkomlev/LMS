...
Что делает. Для каждого курса-родителя, у детей которого в course_uid есть номер раздела
(`tema-<N>` с заголовком «Раздел N»/«Тема N», либо суффикс `-g<N>` с заголовком «Глава N»),
сравнивает фактический порядок (по order_number) с порядком по N. Read-only: ни одного UPDATE.

Второй паттерн («-gN» / «Глава N») до tsk-476 не проверялся вообще — регекс матчил только
`tema-N`, и курсы с семьёй uid вида `wp:inf-7-g4` молча пропускались («другая семантика»,
raw skip) без записи в отчёт. Так были невидимы штатному чеку 2 живых случая того же бага
(курс 826 «Информатика 7 класс», курс 1283 «Тестировщик ПО») — нашли только вручную
отдельным SQL-сканом (tsk-471/tsk-472).
...
# две независимые семьи uid: «tema-N» где угодно в строке, либо «-gN» строго в конце
# (якорь $ обязателен — иначе буква "g" может случайно совпасть в другом месте uid)
_NUM_RE = re.compile(r"tema-(\d+)|-g(\d+)$")
# номер в названии ребёнка: «Раздел 3. …», «Тема 0. …», «Трек 1. Раздел 3. …», «Глава 4. …»
_TITLE_NUM_RE = re.compile(r"(?:Раздел|Тема|Глава)\s+(\d+)")
...
async def main() -> int:
    ...
    for parent_id, parent_uid, parent_title, child_uid, child_title, _order in rows:
        ...
        if not m:
            # ребёнок без номера раздела в uid (ни tema-N, ни -gN) → у родителя другая семантика
            p["skip"] = True
            continue
        n_uid = int(m.group(1) or m.group(2))
