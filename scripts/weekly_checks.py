...
CHECKS: dict[str, Check] = {
    ...
    "cb-drift": Check(
        ...
        takes_quiet=True,
        # Фон здесь нужен: при нуле находок чек печатает счётчики сверки («сверено
        # 1358, разошлось 461, все помечены»), и это единственное место, где видно
        # ОХВАТ. Без них молчаливое сужение — потерялся снимок партии, отвалилась
        # половина ключей — выглядело бы как благополучное «чисто». Проверено на
        # первом живом прогоне 04.09: цифры были отброшены, и убедиться, что сверка
        # прошла по всей базе, оказалось нечем.
        report_on_zero=True,
    ),
