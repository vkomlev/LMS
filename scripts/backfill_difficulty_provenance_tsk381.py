...
def collect() -> tuple[dict[int, dict[str, Any]], dict[int, int]]:
    ...
    # --- Споры внутри канона 1: побеждает более поздний разбор ---
    # Правило оператора 2026-07-23: если разборы в канале расходятся, берём
    # поздний — автор пересмотрел свою же оценку. Даты сняты из
    # `content_hub.source_item.source_updated_at`, посты перечислены целиком,
    # чтобы решение можно было перепроверить.
    for task_id, late_post, late_date, level, all_posts in (
        (2160, "722", "2025-11-23", "сложный", "634 (2025-07-29, средний), 722"),
        (2074, "861", "2026-03-18", "средний", "596 (2025-06-22, сложный), 854, 861"),
        (2290, "720", "2025-11-23", "сложный", "520 (2025-05-21, средний), 720"),
    ):
        provenance[task_id] = _entry(
            1, TG_SOURCE,
            f"поздний разбор — пост {late_post} от {late_date}: {level} "
            f"[разборы: {all_posts}]",
            "tsk-381",
        )
        implied[task_id] = LEVEL_TO_ID[level]
