...
def _markdown_to_html(raw_md: str) -> str:
    ...
    # Курсив — ПОСЛЕ жирного, иначе `**x**` съелось бы как две пары одиночных.
    # Правило годится именно для Markdown Яндекса: умножение он пишет знаком, а не
    # звёздочкой. Для произвольного текста заданий это НЕ так — в базе 41 активное
    # задание с одиночной `*`, и там она почти всегда означает умножение.
    s = re.sub(r"(?<!\*)\*([^*\n]+?)\*(?!\*)", r"<em>\1</em>", s)
    s = s.replace("\xa0", " ")
