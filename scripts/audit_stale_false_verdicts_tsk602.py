...
искать его должен планировщик, а не случайный разбор. Вход планировщика —
``scripts/stale_verdicts_weekly.py`` (журналирование + коды выхода), сама задача
ставится через ``scripts/install_stale_verdicts_check.ps1``.
