...
Решение оператора 16.09 ограничивает пересчёт ЗАДНИМ ЧИСЛОМ ровно записями,
где уже стоит пометка `list_literal` (215 штук) — то же самое исходное
множество, по которому считался масштаб. Механизм фикса общий для всех 16
конструкций каталога (декомпозиция п.2), но общность — про КОД, а не про
объём пересчёта: включать в пересчёт работы, где `list_literal` сейчас нет
вовсе, означало бы заново извлекать код из вложений (`pick_code_for_review`,
`allow_untagged=True`) для строк, где это никогда не проверялось всерьёз —
живой прогон 16.09 показал на такой попытке 16 работ, где пометка
«появлялась» из ничего (не от стема, а от того, что код для старой сдачи
переизвлёкся не тем же способом, что при живой оценке). Уже помеченные
list_literal работы такого риска не несут: хуже, чем есть, там не станет —
либо пометка снимается (наша цель), либо остаётся той же. Вердикт модели,
балл и разбор линтера не трогаются: пересчитывается только ключ
`unseen_constructs`.
...
#: Кандидаты: ТОЛЬКО работы, уже помеченные `list_literal` - решение
#: оператора 16.09 (см. докстринг модуля). Не "все готовые разборы", как у
#: tsk915: там каталог только что расширился, и пересчёт был нужен всем; тут
#: расчёт для конструкций без пометки менять не просили, а сам пересчёт
#: (переизвлечение кода из вложений) ненадёжен там, где раньше не проверялся.
_CANDIDATES_SQL = """
    SELECT tr.id,
           tr.user_id,
           tr.attempt_id,
