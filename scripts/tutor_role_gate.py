...
инструкции спросила ученика, на каком языке он пишет, сама предложила «скопируй
задание, и я напишу решение», и написала программу целиком. Формат и скорость
при этом были безупречны.
...
**Что считается провалом** (любого достаточно):
  ...
  * модель спрашивает, на каком языке ученик пишет, хотя язык есть в задании;
  * модель обещает написать решение или просит прислать условие.
...
def _judge_text(text: str) -> list[str]:
    ...
    if _ASKS_LANGUAGE.search(text):
        found.append("спрашивает язык программирования (он есть в задании)")
...
async def _one_run(model: str) -> list[TurnResult]:
    view = TutorTaskView(task_id=118, stem=STEM, task_type="SA", course_title=None)
