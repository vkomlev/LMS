...
# ВНИМАНИЕ (найдено в tsk-262, аудит 2026-07-17): ЭТА РЕГУЛЯРКА ДЕФЕКТНА.
# Она написана в синтаксисе Python, но исполняется PostgreSQL (`a->>'value' ~ $1`),
# а там `\b` — НЕ граница слова, а символ backspace (граница слова — `\y`).
# Поэтому ветки `\bprint\b|\bimport\b|\bdef\b` не срабатывали НИКОГДА, и задания
# ловились только по скобкам/точке/`=`/квадратным скобкам. Задания с голым
# `import random` (5468, 5617, 5748) не имеют ни одного из этих признаков и
# прошли мимо фикса — у них остался `lower`, то есть `IMPORT RANDOM` продолжал
# засчитываться за `import random`. Проверить можно так:
#   select 'import random' ~ '\bimport\b';  -- false
#   select 'import random' ~ '\yimport\y';  -- true
# Данные уже исправлены в tsk-262 (scripts/set_code_ast_flag_tsk262.py), который
# классифицирует разбором в AST на стороне Python и от регулярок PG не зависит.
# Скрипт оставлен как исторический артефакт: НЕ перезапускать и НЕ копировать
# отсюда CODE_RX. Разбор класса ошибки — docs/ai/ERRORS.md, запись 2026-07-17.
