"""tsk-545 — откат временного назначения курса 1283 пользователю 142
(см. `scripts/tsk545_assign_qa_course_to_user142.py`) после живой проверки.

DELETE FROM user_courses WHERE user_id=142 AND course_id=1283. CASCADE не
трогает `student_course_state`/`student_material_progress` — они не FK на
`user_courses`, поэтому оставшийся прогресс по «Глава 1»/«Глава 2» — безвредный
след живой проверки (согласуется с тем, что 142 — постоянный тест-аккаунт).

Запуск (DSN прод-роли из .mcp.json):
    python scripts/tsk545_unassign_qa_course_from_user142.py                  # dry-run (ROLLBACK)
    DBCHECK_OK=1 python scripts/tsk545_unassign_qa_course_from_user142.py --apply   # COMMIT
"""
...
if __name__ == "__main__":
    ...
    ap.add_argument("--apply", action="store_true", help="Выполнить удаление (COMMIT).")
    args = ap.parse_args()
