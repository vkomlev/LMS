...
#: Сколько ждать сверку. С tsk-852 она читает задания пачками — страницами банка
#: (`GET /api/v1/tasks/`) вместо запроса на каждое: полный прогон по 5089 ключам
#: занимает около минуты против прежних 70 (замеры 09.09.2026). Час здесь —
#: не оценка длительности, а порог «зависло»: он покрывает и деградацию, когда
#: пачка отказала и сверка ушла на запасной путь по одному заданию.
#: Задача планировщика живёт 90 минут.
