...
#: Сколько ждать сверку. Она опрашивает боевой кабинет по одному заданию —
#: около 0,8 с на задание (замеры: 475 заданий за 7:52 04.09.2026; 1000 за 13:30
#: 09.09.2026). С tsk-849 в сверку вошли задания уроков (снимки партий
#: CreateCourses), и охват вырос с ~1360 заданий до ~5090: полный прогон занимает
#: около 70 минут и в прежний час уже не укладывался. Два часа — это снова
#: «зависание, а не медленно»; задача планировщика живёт 150 минут.
#: Ускорение (выдача заданий пачкой по курсу вместо запроса на каждое) вынесено
#: отдельной задачей — после него это значение надо вернуть к часу.
CB_TIMEOUT_SEC = int(os.environ.get("CB_DRIFT_TIMEOUT_SEC", "7200"))
