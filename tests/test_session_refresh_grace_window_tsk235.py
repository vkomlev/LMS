...
- Повтор ПОСЛЕ окна благодати → 401 + отзыв всей цепочки сессий пользователя
  (детект кражи/replay токена сохраняется).
...
async def test_replay_after_grace_window_revokes_chain(db, redis_client):
    ...
    401 + отзыв всей цепочки сессий пользователя (включая преемника)."""
