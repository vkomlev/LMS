...
#: Сколько раз повторить замер, прежде чем признать сложение (tsk-876).
#: Засчитывается ЛУЧШАЯ попытка, и это не смягчение проверки, а разделение
#: двух разных величин: сериализация систематическая — она даёт свои пять
#: удержаний в КАЖДОЙ попытке; чужая нагрузка на машине случайна и только
#: добавляет сверху, поэтому минимум её отсеивает. Обычный прогон стоит одну
#: попытку: как только замер уложился в порог, остальные не выполняются.
_GAP_ATTEMPTS = 3
...
async def test_burst_does_not_serialize_even_when_it_writes(
    ...
    * после tsk-675 — 0,95-1,28 с, вровень с залпом без записи.
    ...
    Одной разницы всё же не хватило (tsk-876). При двух полных прогонах на
    машине разом замер дал 1,79 с против 1,00 с — разницу 0,79 при пороге 0,40,
    хотя в одиночку тот же файл зелёный за шесть секунд. Причина в том, что
    посторонняя нагрузка входит в обе величины НЕ поровну: у залпа с записью
    шагов больше (сам `UPDATE` и его коммит), и каждый лишний шаг под нагрузкой
    дорожает. Поэтому замер повторяется до `_GAP_ATTEMPTS` раз и засчитывается
    лучшая попытка: сложение систематическое и никуда не девается ни в одной
    попытке, а всплеск чужой нагрузки случаен. Обычный прогон по-прежнему стоит
    одну попытку — при удаче цикл заканчивается сразу.
    ...
    # (`_BURST - 1`), так что запас до ложного срабатывания четырёхкратный.
    allowed_gap = _HOLD_SECONDS
    attempts: list[tuple[float, float]] = []

    for _ in range(_GAP_ATTEMPTS):
        ...
        writing_wall, durations = await _burst(
            db_session_factory, student_session["token"]
        )

        attempts.append((free_wall, writing_wall))
        print(
            f"[tsk-675] попытка {len(attempts)}: залп без записи {free_wall:.2f} с "
            f"против {writing_wall:.2f} с с записью "
            f"(разница {writing_wall - free_wall:+.2f} с), "
            f"худший запрос {max(durations):.2f} с"
        )
        if writing_wall - free_wall < allowed_gap:
            break

    best_gap = min(writing - free for free, writing in attempts)
    assert best_gap < allowed_gap, (
        f"залп с записью отстаёт от залпа без неё на {best_gap:.2f} с даже в "
        f"лучшей из {len(attempts)} попыток, допускали меньше {allowed_gap:.2f} с. "
        "Все попытки (без записи → с записью): "
        + "; ".join(f"{free:.2f}→{writing:.2f}" for free, writing in attempts)
        + ". Запись снова выстраивает залп в очередь за строкой сессии — "
        "вернулась авария tsk-621/tsk-675"
    )
