...
# Проверка: CURL-запросы к API; валидация по логам logs/app.log (все 6 операций api.materials_extra);
...
# Требуется: запущенный API (run.py), сеть. Подготовка данных: курсы 7 и 8 — через MCP при необходимости.
...
function Get-Req { param([string]$Path) Invoke-RestMethod -Uri (Url $Path) -Method GET }
...
# 2) POST /materials — создание материала (order_position не передаём — триггер поставит в конец)
$createBody = @{
    course_id = $CourseId
...
# 5) GET /courses/{course_id}/materials — снова список (должен быть новый материал)
try {
...
# 6) POST /courses/{course_id}/materials/reorder
...
# 7) POST /materials/{id}/move — в ту же позицию в том же курсе
try {
...
# 8) POST /courses/{course_id}/materials/bulk-update
try {
...
# 9) POST /materials/{id}/copy — копирование в другой курс
try {
...
# 10) GET /courses/{course_id}/materials/stats
try {
...
# 11) DELETE созданного материала (оставим копию в course_id=8)
try {
    Delete-Req "/materials/$($mid)"
    Pass "DELETE /materials/$($mid)"
} catch { Fail "DELETE /materials/$($mid): $_" }

# 12) Импорт из Google Sheets — пропускаем, пока таблица не подготовлена (-SkipGoogleSheetsImport)
...
# Валидация по логам: в logs/app.log должны быть все 6 операций api.materials_extra (уровень INFO)
$logPath = Join-Path (Split-Path $PSScriptRoot -Parent) $LogFile
$expectedOps = @("list_course_materials", "reorder_materials", "move_material", "bulk_update_materials", "copy_material", "get_course_materials_stats")
