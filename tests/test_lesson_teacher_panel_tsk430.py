"""tsk-430 (Календарь LMS, Фаза 3): панель преподавателя, ad-hoc, reschedule.

Покрывает:
- `GET /teacher/lesson-occurrences`: ownership 403, живой флаг `is_overdue`
  (только для `status='scheduled'`, не для `confirmed`/будущих).
- `POST /teacher/lesson-occurrences/{id}/attendance`: manual_present/absent,
  409 на `rescheduled`.
- `POST /teacher/lesson-occurrences/add-student`: создание ad-hoc, 403
  teacher-mismatch, 422 неверная роль, 409 коллизия.
- `GET /lesson-occurrences/available-slots`, `POST .../reschedule`,
  `POST .../ad-hoc`: часы работы школы, коллизии, перенос помечает старый
  occurrence `rescheduled` и создаёт новый.
"""
...
async def _create_occurrence(
    ...
    occ = LessonOccurrence(
        slot_id=None, student_id=student_id, teacher_id=teacher_id,
        scheduled_at=scheduled_at, duration_minutes=duration_minutes, status=status,
    )
    db.add(occ)
    await db.flush()
    occ_id = occ.id
...
async def test_teacher_list_is_overdue_only_for_scheduled_past(db, client):
    ...
    by_id = {item["id"]: item["is_overdue"] for item in resp.json()}
...
@pytest.mark.asyncio
async def test_add_student_409_collision(db, client):
    teacher_id = await _create_user(db, role="teacher", prefix="tsk430-teach")
    ...
    resp = await client.post(
        ...
        json={
            ...
            # Пересекается: начинается на 30 минут позже первого 60-минутного занятия
...
@pytest.mark.asyncio
async def test_reschedule_creates_new_marks_old_rescheduled(db, client):
    student_id = await _create_user(db, role="student", prefix="tsk430-stud")
    teacher_id = await _create_user(db, role="teacher", prefix="tsk430-teach")
    token, _, _ = await create_session(db, user_id=student_id)
    ...
    assert resp.status_code == 201, resp.text
    new_occ = resp.json()
    assert new_occ["status"] == "scheduled"
    assert new_occ["id"] != old_id

    old_row = (
        await db.execute(
            text("SELECT status, rescheduled_to_id FROM lesson_occurrence WHERE id = :oid"),
            {"oid": old_id},
        )
    ).fetchone()
