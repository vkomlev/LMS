...
SELF_MANAGED_CONNECTION_MODULES: frozenset[str] = frozenset(
    {
        ...
        # tsk-596: та же причина, что у tsk-521 — двойной запуск суточного
        # прохода начислений проверяется только двумя независимыми соединениями.
        # Остальные тесты задачи живут в `test_tsk596_charge_cron.py`.
        "test_tsk596_charge_cron_lock.py",
        # tsk-626: взаимоблокировку на кеше `student_course_state` нельзя
        # воспроизвести на одном соединении — нужны две настоящие параллельные
        # транзакции, захватывающие строки в обратном порядке.
        "test_tsk626_course_state_deadlock.py",
